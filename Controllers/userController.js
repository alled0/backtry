...
const followClub = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Club followed successfully", data: user.followedClubs });
    } catch (error) {
...
const unfollowClub = async (req, res) => {
    ...
    try {
        ...
        if (index === -1) {
            return res.status(400).json({ message: "Club not found in followed clubs" });
        }

        user.followedClubs.splice(index, 1); // Remove the club from followedClubs
        await user.save();

        res.status(200).json({ message: "Club unfollowed successfully", data: user.followedClubs });
    } catch (error) {
...
const joinEvent = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Event joined successfully", data: user.joinedEvents });
    } catch (error) {
...
const leaveEvent = async (req, res) => {
    ...
    try {
        ...
        if (index === -1) {
            return res.status(400).json({ message: "Event not found in joined events" });
        }

        user.joinedEvents.splice(index, 1); // Remove the event from joinedEvents
        await user.save();

        res.status(200).json({ message: "Event left successfully", data: user.joinedEvents });
    } catch (error) {
